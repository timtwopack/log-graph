...
- Добавлена проверка целостности server/standalone артефактов и `build-manifest.json` с SHA-256 исходников.

## Совместимость

Прямое открытие `log-graph-v091.html` всё ещё поддерживается только как generated standalone fallback. Штатный режим — статическая раздача `dist/server`.
