...
- Фоновый trace-precompute передаёт cloned typed arrays в `trace.worker.js` через transfer-list; для очень больших выбранных наборов действует byte-guard, чтобы не удваивать сотни MiB памяти.
