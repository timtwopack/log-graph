...
- To reduce risk, `ColumnarData` keeps an array-like API (`length`, index access, `map/filter/some`, iterator). Some UI code still looks array-oriented, but the backing store is columnar.
...
- Remove remaining temporary object materialization in individual operations (`filter/map` during export, trace-precompute, merge) and gradually move them to direct column operations.
