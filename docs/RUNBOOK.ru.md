...
Это единственный runtime-комплект. Если нужен архив для передачи или отката, выполнить `npm run dist`: скрипт пересоберёт `build`, скопирует его в `dist/log-graph-build-YYYYMMDD-HHMMSS/` и создаст zip рядом. Не класть рядом реальные технологические логи, если их не нужно передавать явно. Сессии лучше переносить отдельными `.pagraph.json.gz` через меню приложения.
